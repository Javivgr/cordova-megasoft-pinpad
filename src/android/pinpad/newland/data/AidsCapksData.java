 ...
 * segun el formato requerido por el Newland N910
 * 
 * 
...
public class AidsCapksData {
	...
	//Metodos Privados
	/**
	 * metodo que contruye el tag solicitado 
	...
	//Metodos Publicos
	/**
	 * metodo que prepara la informacion de los AIDS de las marcas 
	 * para su carga en el N58
	 * @param aids (List[BeanAidsInfo]) lista de aids a cargar 
	 * @return (ArrayList<String>) todos los aids formateados segun el requerimiento del n910
	 */
	public static ArrayList<String> buildAidsInfo(List<BeanAidsInfo> aids){
		
		//inicializamos los elementos requeridos para la construccion de los AIDS
		StringBuffer sb = new StringBuffer();
		ArrayList<String> aidsList = new ArrayList<String>();
		...
		for(BeanAidsInfo aid : aids){
			...
			//agregamos un filtro de values 'null'
			if (!aid.getFloorLimit().equals("null")){sb.append(buildTag(terminalFloorLimit, aid.getFloorLimit()));}
			if (!aid.getThresholdValue().equals("null")){sb.append(buildTag(thresholdValue, aid.getThresholdValue()));}
			if (!aid.getMaximumTargetPercentage().equals("null")){sb.append(buildTag(maximumTargetPercentage,aid.getMaximumTargetPercentage() ));}
			if (!aid.getTargetPercentage().equals("null")){sb.append(buildTag(targetPercentage, aid.getTargetPercentage()));}
			sb.append(buildTag(defaultTdol, aid.getDDOL()));
			sb.append(buildTag(bOblinePin, "00"));

			//seteamos el código del país (Transaction Country Code tag)
			sb.append(buildTag("9F1A","0862"));

			//agregamos string individual de aid a lista
			aidsList.add(sb.toString());
			//reseteamos StringBuffer
			sb.setLength(0);
		}
		
		//retornamos la lista de AIDS construidos
		return aidsList;
	}
	
	/**
	 * metodo que prepara la informacion de las llaves EMV de las marcas 
	 * para su carga en el N58
	 * @param capks (List[BeanEmvKeyInfo]) lista de llaves EMV a cargar
	 * @return (ArrayList<String>) todos las capks formateados segun el requerimiento del n910
	 * @throws ParseException - no logro parsear la fecha recibida de la llave
	 */
	@SuppressLint("DefaultLocale")
	public static ArrayList<String> buildCapksInfo(List<BeanEmvKeyInfo> capks) throws ParseException{

		//inicializamos los elementos requeridos para la construccion de los CAPKS
		StringBuffer sb = new StringBuffer();
		ArrayList<String> capksList = new ArrayList<String>();

		for(BeanEmvKeyInfo info : capks){

			/* if (!info.getHash().equals("null")){capks.next();} */
			...
			if (!info.getHash().equals("null")){sb.append(buildTag(hash, info.getHash()));}
			
			//agregamos string individual de capk a lista
			capksList.add(sb.toString());
			//reseteamos StringBuffer
			sb.setLength(0);
		}
		
		//retornamos la lista de CAPKS construidos
		return capksList;		
	} 
